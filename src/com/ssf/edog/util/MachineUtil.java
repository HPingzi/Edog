...
public class MachineUtil {
	...
	public int[] compareDate(long time1, long time2) {
		...
		ls[0] = (int) hours;
		ls[1] = (int)minutes;
		PjLog.d("SSF", " ++++ " + days + " days " + hours + " hours " + minutes+ " minutes " + seconds + "seconds" + "\n");
		return ls;
	}	
	...
	// 设置开关机
	private int setPowerOnOff(byte on_h, byte on_m, byte off_h, byte off_m, byte enable) {
		int fd, ret;
		...
		if (fd < 0) {
			...
			if (ret != 0) {
				...
				if (3 == enable) {
					PjLog.d("SSF", "定时开关机：开 距离关机时间还有"+off_h+":"+off_m);
					if (mOnStateListener != null) {
						mOnStateListener.onSccessful("定时开关机：开 距离关机时间还有"+off_h+":"+off_m);
					}
				} else if (0 == enable) {
					PjLog.d("SSF", "定时开关机：关");
					if (mOnStateListener != null) {
